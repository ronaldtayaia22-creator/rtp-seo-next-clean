...
const TermsConditions = ({ language }: TermsConditionsProps) => {
  const t = getTranslation(language).termsConditions;
    ...
    <div className="min-h-screen pt-32 pb-20 px-4">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="space-y-8 text-foreground/90">
          ...
          <section>
            <h2 className="text-2xl font-semibold mb-4 text-primary">{t.section9.title}</h2>
            <p className="mb-4">{t.section9.description}</p>
            <p className="mb-4">{t.section9.exceptions}</p>
            <ul className="list-disc list-inside space-y-2 ml-4">
              {t.section9.exceptionList.map((exception, index) => (
                <li key={index}>{exception}</li>
              ))}
            </ul>
            <p className="mt-4">{t.section9.duration}</p>
          </section>

          <section>
            <h2 className="text-2xl font-semibold mb-4 text-primary">{t.section10.title}</h2>
            <ul className="list-disc list-inside space-y-2 ml-4">
